...
class TelemetryCacheTest {
    ...
    public void directMmtcSqliteCacheDbFrameSampleQueryRangeTest() throws Exception {
        ...
        jdbi.withHandle(handle -> {
            MmtcSqliteCacheDb.writeAllFrameSampleCoveredQueryRanges(handle, originalRanges);
            return true;
        });

        assertEquals(
                originalRanges,
                jdbi.withHandle(handle -> MmtcSqliteCacheDb.readAllFrameSampleCoveredQueryRanges(handle))
        );
        ...
        jdbi.withHandle(handle -> {
            MmtcSqliteCacheDb.writeAllFrameSampleCoveredQueryRanges(handle, newOriginalRanges);
            return true;
        });
    ...
    public void directMmtcSqliteCacheDbFrameSampleTest() throws Exception {
        ...
        jdbi.withHandle(handle -> {
            MmtcSqliteCacheDb.writeFrameSamples(handle, originalSamples);
            return true;
        });

        assertEquals(
                originalSamples,
                jdbi.withHandle(handle -> MmtcSqliteCacheDb.readFrameSamples(handle, new OffsetDateTimeRange(odt("2024-360T00:00:00"), odt("2024-361T00:00:00"))))
        );
    }
