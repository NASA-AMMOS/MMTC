...
public class TimeCorrelationApp {
    ...
    public TimeCorrelationApp(String... args) throws Exception {
        try {
            this.config = new TimeCorrelationRunConfig(new TimeCorrelationCliInputConfig(TimeCorrelationRunConfig.TargetSampleInputErtMode.RANGE, args));
    ...
    private CorrelationTriplet getLookbackRecordForPredictedClkChgRate(Double tdtGOfNewTriplet) throws MmtcException, TextProductException, TimeConvertException {
        ...
        if (desiredPriorCorrelationTdt.isPresent()) {
            ...
            Double lookbackRecTdtG = mostRecentLookbackRecMeetingMinimumOnly.getTdt();
            ...
            errorMsg       += "The most recent lookback record in the input SCLK Kernel is at TDT(G) = " + mostRecentLookbackRecMeetingMinimumOnly.getTdtCalStr() + ",";
    ...
    private Double computePredictedClkChgRate(Integer sclk, Double tdt_g) throws TextProductException, TimeConvertException, MmtcException {
        ...
        Double priorEncSclk  = lookBackRec.getEncSclk();
        ...
        Double tdt_g0        = lookBackRec.getTdt();
    ...
    private Double computeInterpolatedClkChgRate(Integer sclk, Double tdt_g) throws TextProductException, TimeConvertException {
        ...
        double priorEncSclk = lastRecValue.getEncSclk();
        int sclk0           = TimeConvert.encSclkToSclk(naifScId, sclk_kernel_fine_tick_modulus, priorEncSclk).intValue();
        double tdt_g0       = lastRecValue.getTdt();

        logger.debug("computeInterpolatedClkChgRate(): Prior enc SCLK = " + Double.toString(priorEncSclk));
        logger.debug("computeInterpolatedClkChgRate(): Prior TDT(G) str = " + lastRecValue.getTdtCalStr());
                ...
                case COMPUTE_INTERPOLATE:
                    ...
                    ctx.correlation.updatedInterpolatedTriplet.set(
                        new CorrelationTriplet(
                                ...
                                currentFinalTriplet.tdt,
                                new BigDecimal(ctx.correlation.interpolated_clock_change_rate.get())
                        )
                    );
                ...
                final int curLatestTripletPartition = ctx.config.getSclkPartition(
                        TimeConvert.parseIsoDoyUtcStr(
                            TimeConvert.tdtCalStrToUtc(
                                    ctx.currentSclkKernel.get().getLastTriplet().getTdtCalStr(),
                                    9
                            )
                        )
                );
    ...
    private static void ensureIncreasingTdtAndSclkCorrelationValues(TimeCorrelationContext ctx) throws TextProductException, TimeConvertException, MmtcException {
        ...
        final String prevTdtGStr = ctx.currentSclkKernel.get().getLastTriplet().getTdtCalStr();
        final double prevTdtG = ctx.currentSclkKernel.get().getLastTriplet().getTdt();
        ...
        final double prevEncSclk = ctx.currentSclkKernel.get().getLastTriplet().getEncSclk();
