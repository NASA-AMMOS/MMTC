...
public class BuiltInOutputProductMigrationManager {
        ...
        if (Files.exists(config.getRunHistoryFilePath())) {
            ...
            if (runHistoryCsv.hasColumn(RunHistoryFile.MMTC_BUILT_IN_OUTPUT_PRODUCT_VERSION)) {
                ...
                if (!mmtcProdVersion.isPresent()) {
                    throw new IllegalStateException("Unexpected lack of MMTC built-in output product version in Run History File");
                }
    ...
    public void migrate() throws MmtcException {
        String versionOfExistingProducts = getVersionOfExistingProducts();
        ...
        // todo create backup (please enter a path to store backups of output files), then require confirmation
        // prompt for a filepath, or Ctrl+C to quit
        // get all output files, and compress them to target filepath
