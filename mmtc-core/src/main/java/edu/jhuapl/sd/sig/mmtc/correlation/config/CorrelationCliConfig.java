 ...
 */
public class CorrelationCliConfig {
    protected HelpFormatter help = new HelpFormatter();
    protected Options opts = new Options();
    protected CommandLine cmdLine;
    private String[] args;

    private MmtcConfig.ClockChangeRateMode clockChangeRateMode;
    ...
    private final String baseCliUsage;
    ...
    public CorrelationCliConfig(Mode mode, String[] args) {
        this(mode, args, Collections.emptyList());
    }

    public CorrelationCliConfig(Mode mode, String[] args, Collection<Option> additionalOptions) {
        switch (mode) {
            case CORRELATE:
                baseCliUsage = "mmtc correlate [options] <start-time> <stop-time>";
                break;
            case AUTOCORRELATE:
                baseCliUsage = "mmtc autocorrelate [options] [up to time]";
                break;
            default:
    ...
    public boolean load() {
        try {
            ...
            if (isHelpSet()) {
                help.printHelp(baseCliUsage, opts);
                System.exit(0);
            }
