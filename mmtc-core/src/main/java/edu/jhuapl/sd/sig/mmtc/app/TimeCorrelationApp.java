...
public class TimeCorrelationApp {
    ...
    private TimeCorrelationTarget buildTimeCorrelationTargetWithinSampleSet() throws MmtcException {
    ...
    private void recordRunHistoryFilePreRunValues() throws MmtcException {
        ...
        // Output products
        for (OutputProductTypeDefinition prodDef : OutputProductTypeDefinition.all()) {
    ...
    public void run() throws Exception {
        ...
        // log general information and perform some sanity checks
        {
        ...
        final TimeCorrelationTarget tcTarget = buildTimeCorrelationTargetWithinSampleSet();
        ...
        // log general information about the telemetry that will be used for time correlation
        {
        ...
        // mmtc-10 todo, review telemetry source connection and disconnection pattern

        // mmtc-10 get all ancillary information
        new TimeCorrelationAncillaryOperations(ctx).gatherAndComputeAdditionalMetrics();

        // set new SCLK version string
        {
            ctx.newSclkVersionString.set(getNextSclkKernelVersionString());
        }

        // mmtc-10 write custom output products
        for (OutputProductTypeDefinition prodDef : OutputProductTypeDefinition.all()) {
            final String postRunColProdColName = RunHistoryFile.getPostRunProductColNameFor(prodDef);

            if (prodDef.shouldBeWritten(ctx)) {
                final OutputProductTypeDefinition.ProductWriteResult res = prodDef.write(ctx);
