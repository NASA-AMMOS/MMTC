...
=== Installing from a compressed distribution archive

MMTC is also distributed as a .zip and a .tar.gz archive file. Either of these methods of installation can be used when RPM installation is not desired or possible.
...
=== Installing and running MMTC via a container image

The project also produces OCI container images, built for use with e.g. podman, Docker, etc.  Two images are produced: one with the MMTC CLI application only, and one with both the CLI application and web application.  These images have MMTC installed under /opt/local/mmtc, with subdirectories matching the 'nominal MMTC installation' description given above.  It is expected that users deploying MMTC with these container images will mount configuration into the container appropriately and configure persistence as desired for logs and output files.

The following example demonstrates running the MMTC CLI from its container image using podman:

[source, bash]
----
podman run --it -rm localhost/mmtc:x.y.z 2026-001T00:00:00 2026-007T00:00:00
----

The following example demonstrates running the MMTC web application from its container image using podman:

[source, bash]
----
podman run --it -rm -p 8080:8080 localhost/mmtc-webapp:x.y.z
----
...
|telemetry.cacheFilePath
|OPTIONAL
|STR
|The filepath where MMTC should maintain its telemetry cache.  If a value is given, MMTC will begin caching time correlation telemetry it retrieves from the telemetry source to a sqlite file at this location.  If a value is not specified for this key, MMTC will perform no telemetry caching.
...
|telemetry.exactTargetSampleSelection.targetToSupplementalQueryWindowMin
|OPTIONAL
|INT
|Only used when a specific frame's target ERT is selected.  This represents the 'grace' period around the target frame that will be included in the query to the configured telemetry source so that it can gather sufficient data to match up the target frame's ERT and the supplemental frame's packet from a telemetry source-agnostic position.  Defaults to 5 minuteus.
...
|OPTIONAL
|String
|The path where an executing instance of MMTC will maintain a lock file to avoid multiple copies of MMTC interfering with each other.  If not set, defaults to a dotfile in the same directory as the Run History File.
...
|table.timeHistoryFile.oscTempFractionDigits
|OPTIONAL
|INT
...
|product.sclkScetFile.missionId
|OPTIONAL
|INT
|The Mission ID to be included in the SCLK/SCET file header. Required if product.sclkScetFile.create is `true`.
...
|OPTIONAL
|STR
|The basename of Uplink Command Files. For example, given the filename “uplinkCmd1577130458.csv”, “uplinkCmd” is the basename. Required if product.uplinkCmdFile.create is `true`.

|product.uplinkCmdFile.writeHeaders
|OPTIONAL
|BOOL
|Whether to write headers or not.  If set to `true`, headers will be written to Uplink Command Files.  If set to `false`, they will not be written.  Default value is `false`.

|product.uplinkCmdFile.versionSuffixMode
|OPTIONAL
|STR
|There are two modes, specified as strings, by which MMTC can version uplink command files.  One mode `appRuntimeTimestamp`, which will append an integer timestamp to the end of each file.  The other mode is `sclkVersion` which will append the same SCLK version string as the new kernel that was written with the correlation.  Default value is `appRuntimeTimestamp`.

|product.parameterGroupUpdateFile.create
|OPTIONAL
|STR
|Activates creation of the Parameter Group Update File. Set to `true` if this file is to be created. Valid values are `true` or `false`.  Default value is `false`.

|product.parameterGroupUpdateFile.create
|CONDITIONAL
|STR
|Specifies the path at which the Parameter Group Update File will be written and appended to.  Required `product.parameterGroupUpdateFile.create` is set to `true`.
...
|telemetry.source.plugin.ampcs.defaultFrameSizeBits
|OPTIONAL
|INT
|The default size of a mission telemetry frame containing time correlation data in bits. This value is used if MAPPING variable is not set.  Frame length is used for calculating downlink data rates if the packet parser configuration is not set to read downlink data rates from packets, as well as in reporting.  This is the entire ‘physical’ size of the transfer frame, including headers and trellis bits, and taking into account encoding schemes.  Must be a positive integer.
...
The Input SCLK kernel is the SCLK kernel that MMTC created the previous time that it ran. Each time MMTC runs, it creates a new SCLK kernel that contains all the contents of the previous one with a new time correlation record added to it. This new SCLK kernel is the input SCLK kernel for the next run. When MMTC runs, it looks for SCLK kernels in the directory indicated in the *spice.kernel.sclk.kerneldir* configuration parameter that match the naming patterns given in the *spice.kernel.sclk.baseName*, *spice.kernel.sclk.separator*, and *spice.kernel.sclk.suffix* configuration parameters (<basename><separator><nnnnn>.<suffix>) where nnnnn is an integer version number. By SPICE conventions, SCLK kernel filenames should have a suffix of “.tsc”. MMTC selects the matching kernel file with the highest version number. It assumes that this is the one from the previous run and loads it. The new kernel that it produces will have this name, but with the version number incremented.
...
==== Parameter Group Update File

The Parameter Group Update File is an optional operations product that can be used in the process of planning commands to uplink time correlation data to the spacecraft. It is a CSV formatted text file that is appended to with each successful time correlation execution. It contains the same values as an Uplink Command File noted above, with the addition of time correlation execution metadata.  The fine time SCLK (i.e., subseconds) is not included in this file because the corresponding ground time is adjusted, by an amount equal to the fine time, to a whole tick SCLK coarse time; in other words, the subseconds are subtracted from the TDB and TDT ground times. The SCLK1, TDT1, and CLKCHGRATE1 values which the flight software uses to compute ground time from SCLK derive from these. It is left to other elements of the space mission ground system to convert the values in this file to the necessary format and load them into a command for uplink. For missions that use this product, it is left to the mission’s GDS to take this file and form its contents into a command that can be uplinked. See Appendix H for a sample of this file.

This file is produced if the *product.parameterGroupUpdateFile.create* configuration parameter is set to `true`.
...
MMTC offers a built-in feature to create an independent copy of itself at another location.  To invoke this functionality, run the following:

```
bin/mmmtc create-sandbox <path-to-new-mmtc-location>
```

Afterwards, you may switch directories to the new path and use that independent copy of MMTC for testing configuration changes, anomaly resolution, etc.

== Telemetry Caching

MMTC supports optional telemetry caching.  This is helpful when rerunning MMTC over the same time period of telemetry, especially in the following cases:

- during adaptation, integration, and testing
- for use with the MMTC UI
- when adjusting configuration during anomaly resolution or otherwise

To enable telemetry caching, populate the configuration key `telemetry.cacheFilePath` with a filepath.  MMTC will begin maintaining a cache of time correlation telemetry in a sqlite cache at this path.

MMTC also offers two commands to interact with the cache:
- `mmtc precache <start time> <stop time>`: pre-load the cache with telemetry over the specified time range
- `mmtc cache-stats`: log information about the contents of the telemetry cache

To clear the cache, simply delete this file when MMTC is not running.
...
MMTC 1.6.0 introduces a web application that is packaged separately alongside the traditional MMTC CLI application.  It can be used to:

* Plot time correlation telemetry (including their SCET error) and correlation triplets (SCLK kernel records) over time 

* Create new correlations manually, choosing exactly which telemetry points and prior correlation records to use, along with MMTC's standard correlation options

* View and download MMTC output products


This web application is currently intended for use by a single user at a time, to be started when it is needed, and exited when the user's tasks are complete.
...
|webapp.leapSecondsKernelPath
|REQUIRED
|STR
|The web application requires a leap second kernel for operation.  Please specify the path to a leapsecond kernel (LSK) in this configuration entry.

|webapp.auth.mode
|REQUIRED
|STR
|Either 'NONE' or 'AUTOGEN_BASIC_HTTP_AUTH'.  'NONE' provides no authentication and should be used with care.  'AUTOGEN_BASIC_HTTP_AUTH' protects access to a local running will create a unique passcode at each startup, printed to the console, to be used in combination with the username 'mmtc' when accessing the web application.

|webapp.serve.plaintext.enabled
|REQUIRED
|STR
|Whether to serve the application on a TCP port in plaintext.  Either 'true' or 'false'.

|webapp.serve.plaintext.port
|OPTIONAL
|INT
|If webapp.serve.plaintext.enabled is set to 'true', this is the port at which the MMTC web app will be served.

|webapp.serve.tls.enabled
|REQUIRED
|STR
|Whether to serve the application on a TCP port via TLS.  Either 'true' or 'false'.

|webapp.serve.tls.port
|OPTIONAL
|INT
|If webapp.serve.tls.enabled is set to 'true', this is the port at which the MMTC web app will be served.

|webapp.serve.tls.keystore.location
|OPTIONAL
|STR
|If webapp.serve.tls.enabled is set to 'true', this is the path that will be used ƒor the server cert.  Must be PKCS12 format.

|webapp.serve.tls.keystore.password
|OPTIONAL
|STR
|If webapp.serve.tls.enabled is set to 'true', this is the passphrase that will be used to access the aforementioned server cert.

|webapp.auth.timeout
|OPTIONAL
|INT
|The number of hours after which an MMTC login session will time out.

|webapp.testmode.enabled
|OPTIONAL
|STR
|If set to 'true', then MMTC will use no ephemerides for OWLT calculations.

|webapp.testmode.owltSec
|OPTIONAL
|INT
|If webapp.testmode.enabled is set to 'true', this is the value in seconds that will be used by default for OWLT calculations in place of calculating using ephemerides.
|===

It is also heavily recommended that telemetry caching is enabled when using the web application, which can dramatically improve performance.  Please see the documentation for the key `telemetry.cacheFilePath` for more information.

=== Starting and accessing the MMTC web application

To start the web application, ensure you've installed or unpacked the appropriate MMTC RPM or zip distribution, and run:

```
bin/mmtc-webapp
```

Open a browser, and visit an appropriate URL that corresponds to your chosen configuration, e.g.:

* for a plaintext configuration, a typical URL may be http://localhost:8080

* for a TLS configuration, a typical URL may be https://localhost:8443

If necessary, log in with the generated basic HTTP auth credentials.

=== Using the MMTC web application

To use the application:

* Navigate the areas of the application using the sidebar on the left
* The chart on the main page displays retrieved time correlation telemetry points
** The range (in ERT) can be adjusted using the calendar controls on the top left
** The SCLK kernel used to calculate SCET Error values and to display the correlation triplets can also be chosen from the top left
** Use the bar to pan & zoom the chart within the queried time range
** Hover over any point on the chart for more details about the data point
* To create a new correlation:
** Click the 'New correlation' button
** Configure the inputs as desired
** Click 'Preview' to preview the effect of the new correlation run without impacting any files
*** If you'd like to make revisions, click 'Back'
*** If you'd like to commit the correlation to the output products, click 'Commit'
* To roll back the state of MMTC's output products to a prior state:
** Navigate the table below the chart and use the arrow buttons to choose which state you'd like to restore

To exit the application, send a SIGINT (e.g., via Ctrl+C in a shell environment) to gracefully shut down the application.
...
[[table9, Table 9: Time History File Fields]]
[%autowidth]
|===
...
[[table10, Table 10: Raw Telemetry Table Fields]]
[%autowidth]
|===
| Name | Typical Value | Remarks
...
=== H. Sample Parameter Group Update File
[[appendix_H, Appendix H: Sample Parameter Group Update File]]

An example of a Parameter Group Update File as described in <<Parameter Group Update File>> created from New Horizons data.

----
Run Time,Run ID,Associated SCLK Kernel Version,Coarse SCLK,ET,TDT,TDT Calendar,Clock Change Rate (s/s)
2026-222T14:05:44.927000,00001,1001,375043344,566009494.949427,566009494.950158,08-DEC-2017-12:51:34.950158,1.00000001771
2026-222T14:05:49.003000,00002,1002,375140538,566106688.950705,566106688.951407,09-DEC-2017-15:51:28.951407,1.00000001704
2026-222T14:05:53.285000,00003,1003,375226885,566193035.952154,566193035.952830,10-DEC-2017-15:50:35.952830,1.00000001792
2026-222T14:05:57.260000,00004,1004,375288078,566254228.953176,566254228.953833,11-DEC-2017-08:50:28.953833,1.00000001779
----

=== I. References
[[appendix_I, Appendix I: References]]
...
=== J. Acronyms and Glossary
[[appendix_J,Appendix J: Acronyms and Glossary]]
